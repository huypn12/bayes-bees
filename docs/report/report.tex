...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[->, >=stealth', auto, semithick, node distance=3cm]
    \tikzstyle{every state}=[fill=white,draw=black,thick,text=black,scale=1]
    \node[state]    (A)                     {init};
    \node[state]    (B)[right of=A]         {$0,0,1$};
    \node[state]    (C)[above of=B]         {$0,1,1$};
    \node[state]    (D)[below of=B]         {$1,1,1$};
    \node[state]    (E)[below of=D]         {$0,0,0$};
    
    \node[state]    (F)[right of=B]         {$1,-1,-1$};
    \node[state]    (G)[right of=D]         {$1,1,-1$};
    \node[state]    (H)[right of=F]         {$0 1 1$};
    \node[state]    (I)[right of=B]         {$1,-1,-1$};
    \node[state]    (J)[right of=D]         {$1,1,-1$};
    \node[state]    (K)[right of=F]         {$0 1 1$};
    \node[state]    (L)[right of=B]         {$1,-1,-1$};
    \path
(A) edge     node{$3r_0(1-r_0)^2$}      (B)
...
%% TODO: example of data synthesis
...
To measure \textit{accuracy}, the distance from estimated parameter to true parameter, we use Root
Mean-Squared Error.
